...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file']) && $id > 0) {
    ...
    if (move_uploaded_file($fileTmp, $filePath)) {
        $sql = $conn->prepare("INSERT INTO cursos_imagens (curso_id, nome_arquivo) VALUES (?, ?)");
...
if (isset($_GET["delete"])) {
    ...
    $sql = $conn->prepare("SELECT nome_arquivo FROM cursos_imagens WHERE id=?");
    $sql->execute([$imgId]);
    $img = $sql->fetch();
    ...
    $sql = $conn->prepare("DELETE FROM cursos_imagens WHERE id=?");
    $sql->execute([$imgId]);

    header("Location: cursos-listar.php?id=$id&listar=1");
    exit;
}
?>
...
<body>
        ...
        const dz = new Dropzone("#albumDropzone", {
            ...
            init: function() {
                this.on("queuecomplete", function() {
                    ...
                    window.location.href = "cursos-listar.php?id=<?= $id ?>&listar=1";
                });
            }
        });
    </script>
    ...
    <?php if ($listar): ?>
        ...
        <div>
            <?php
            $sql = $conn->prepare("SELECT * FROM cursos_imagens WHERE curso_id=?");
            $sql->execute([$id]);
            while ($img = $sql->fetch()):
            ?>
                <div class="img-box">
