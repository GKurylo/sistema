...
if ($id) {
    ...
    $sql = $conn->prepare("UPDATE cursos 
        SET nome = ?, descricao = ?, tipo = ?, status = ?, locais_ids = ?, imagem = ?, matriz = ?, planodecurso = ? 
        WHERE id = ?");
    $resultado = $sql->execute([$nome, $descricao, $tipo, $status, $locais_string, $imagem, $matriz, $plano, $id]);
} else {
    // Insere novo curso
    $sql = $conn->prepare("INSERT INTO cursos 
        (nome, descricao, tipo, status, locais_ids, imagem, matriz, planodecurso) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $resultado = $sql->execute([$nome, $descricao, $tipo, $status, $locais_string, $imagem, $matriz, $plano]);
}
